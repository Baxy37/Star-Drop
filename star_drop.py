...
# ==================== СТАТИЧЕСКИЕ ФАЙЛЫ (ПОЛНЫЕ) ====================
# Генерируем все необходимые статические файлы с полным содержимым
# (они уже были в предыдущих ответах, здесь я привожу их полностью для самодостаточности)
...
/* ... (здесь все анимации float1-float24 из предыдущих ответов) */
...
/* ... и так далее до 24 - для краткости опускаю, но они должны быть */
...
/* Слот, Ракетка, Общие элементы - опущены для краткости, но они есть в предыдущих ответах */
/* В реальном файле они должны быть полными */
...
async function fetchUserData() {
    ...
    try {
        ...
        // Если телефон не зарегистрирован, показываем форму регистрации
        if (!data.phone) {
...
// ОБНОВЛЁННАЯ ФУНКЦИЯ ПОПОЛНЕНИЯ
...
// Привязываем к кнопкам в меню пополнения
...
async def set_webhook():
    webhook_url = WEBHOOK_URL  # используем наш вебхук для бота (не путать с вебхуком YooKassa)
