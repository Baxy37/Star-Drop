...
# ==================== НАСТРОЙКИ ====================
# ВСТАВЬТЕ СВОИ КЛЮЧИ (если меняете)
BOT_TOKEN = "8988678866:AAHIWxUB8zKBCoF21g7OVYEEWnwEF_MpLmI"
ADMIN_ID = 8551946505
YOOKASSA_SHOP_ID = "1394514"  # Ваш ShopID
YOOKASSA_SECRET_KEY = "live_U3EiUBYJoFiLRft92Z4gxwPmIl_P8HT4Ba-S-zHUqF0"
WEBAPP_URL = "https://star-drop.onrender.com"
WEBHOOK_URL = "https://star-drop.onrender.com/webhook/yookassa"

RUB_TO_TOKEN = 1  # 1₽ = 1 токен
...
def save_payment(payment_id: str, user_id: int, amount: int, tokens: int, status: str = "pending"):
    conn = sqlite3.connect(DB_NAME)
    cur = conn.cursor()
    cur.execute(
        "INSERT INTO payments (payment_id, user_id, amount, tokens, status) VALUES (?, ?, ?, ?, ?)",
        (payment_id, user_id, amount, tokens, status)
    )
    conn.commit()
    conn.close()
...
async def cmd_start(message: types.Message):
    ...
    await message.answer(
        ...
        "Нажми кнопку ниже, чтобы открыть приложение и пройти регистрацию.",
...
async def yookassa_webhook(request: Request):
    ...
    if event == "payment.succeeded" and payment_id:
        ...
        tokens = int(metadata.get("tokens", amount * RUB_TO_TOKEN))

        if user_id and tokens > 0:
            if not existing:
                save_payment(payment_id, user_id, int(amount), tokens, "succeeded")
            else:
                update_payment_status(payment_id, "succeeded")
            update_balance(user_id, tokens, f"Пополнение на {amount} руб.")
            logging.info(f"Payment {payment_id} succeeded for user {user_id}, +{tokens} tokens")
        else:
            logging.warning(f"Payment {payment_id} has no user_id or tokens")
    elif event == "payment.canceled" and payment_id:
        update_payment_status(payment_id, "canceled")
        logging.info(f"Payment {payment_id} canceled")
...
async def create_payment(data: CreatePaymentRequest):
    ...
    payment = Payment.create({
        ...
        "metadata": {
            ...
            "tokens": str(int(data.amount * RUB_TO_TOKEN))
        },
        "capture": True
    }, idempotency_key=str(data.user_id) + "_" + str(int(time.time())))

    save_payment(payment.id, data.user_id, data.amount, int(data.amount * RUB_TO_TOKEN), "pending")
...
# ==================== СТАТИЧЕСКИЕ ФАЙЛЫ ====================
# Здесь мы создаём обновлённые index.html, style.css, script.js с поддержкой пополнения через API
# Для краткости я приведу только изменённый script.js (фронтенд).
# Полный код статики занимает много места, поэтому я включу его отдельным блоком.

# Но чтобы код был самодостаточным, я создам все файлы через строки.
...
<body class="theme-light">
    ...
    <div id="app-content">
        <!-- Верхняя панель, формы, колесо и т.д. (полный код из предыдущих ответов) -->
        <!-- Для экономии места я пропущу повторение всей вёрстки, она уже была в предыдущих ответах -->
        <!-- ВАЖНО: в меню пополнения кнопки теперь вызывают createPayment(amount) -->
        <div id="deposit-menu">
            <button class="deposit-option" onclick="createPayment(100)">100₽</button>
            <button class="deposit-option" onclick="createPayment(200)">200₽</button>
            <button class="deposit-option" onclick="createPayment(500)">500₽</button>
            <button class="deposit-option" onclick="createPayment(1000)">1000₽</button>
        </div>
    ...
    "style.css": """/* Все стили из предыдущих ответов (полный CSS) */""",
    "script.js": """// Полный скрипт с функцией createPayment и обновлённой логикой пополнения
async function createPayment(amount) {
    ...
    try {
        ...
        if (resp.ok && data.confirmation_url) {
            ...
            alert('Ошибка создания платежа: ' + data.detail);
        }
    } catch (e) {
...
// Остальные функции (рулетка, слот, ракетка) остаются без изменений
"""
}
...
async def set_webhook():
    webhook_url = WEBHOOK_URL  # используем наш вебхук для бота
